<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" import="beans.*, java.text.SimpleDateFormat"%>
					...
					<table class="table">
						...
						<tr>
							<td>
								...
								<div class="col-md-2">
									<fmt:formatDate value="${article.h_date}" timeStyle="medium"
						...
						<tr>
							<td>
								<div class="col-md-12 contentarea" align="left">${fn:replace(article.h_body, replaceChar, "<br/>")}</div>
							</td>
						</tr>
						<tr>
							<td>
										...
										<c:if test="${!empty mem_id}">
											...
											<c:if
												...
												<button type="button" class="btn btn-warning"
													onclick="document.location='H_UpdateForm.do?h_number=${article.h_number}&pageNum=${pageNum}'">수정</button>
												<button type="button" class="btn btn-danger"
													onclick="document.location='H_DeleteForm.do?h_number=${article.h_number}&h_title=${article.h_title}&h_mem_id=${article.mem_id}&pageNum=${pageNum}'">삭제</button>
											</c:if>
										</c:if>
									</div>
								</div>
											...
											<div class="col-md-2" align="left">
												<c:if test="${comment.h_cnickname!=' '}">
													<font style="font-size: 0.8rem"> <fmt:formatDate
															value="${comment.h_cdate}" timeStyle="medium"
															pattern="yy.MM.dd  HH:mm" />
													</font>
												</c:if>
											...
											<div class="col-md-12" align="left">
												<c:if test="${comment.h_cnickname!=' '}">
													...
													<c:if test="${comment.mem_id==mem_id || mem_id.equals('admin')}">
														<button type="button" class="btn btn-default btn-xs"
															onclick="location.href='H_CommentDeleteForm.do?h_cnumber=${comment.h_cnumber}&h_number=${article.h_number}&h_cref=${comment.h_cref}&h_cre_level=${comment.h_cre_level}&h_cmem_id=${comment.mem_id}'">삭제</button>
													</c:if>
												</c:if>
												...
												<c:if test="${!empty mem_id}">
													...
													<div class="hiddenText" id="a${comment.h_cnumber}">
														<!-- hiddenText -->
														<form id="hrecommentform" role="form" class="form-horizontal" method="post" name="hrecommentform" action="H_CommentWriteProc.do">
